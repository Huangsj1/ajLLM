...
def main():
    ...
    parser.add_argument("--initial-token-budget", type=int, default=32)
    ...
    budget = MemoryBudget(
        ...
        initial_token_budget=args.initial_token_budget,
    )
    engine = Engine(runner, budget.config)
    engine.set_token_budget(budget.stats.token_budget)
    service = EngineService(engine, budget, max_pending_requests=args.max_pending_requests)
    print(json.dumps({"resolved_engine": asdict(budget.config), "memory": budget.snapshot()}), flush=True)
