...
from ajllm.utils.vllm_export import iter_qwen3_named_parameters
...
def generate_completions(
    vllm_base_url: str,
    model_id: str,
    prompts: list[list[int]],  # exact, already-rendered policy token IDs
    sampling_params: dict,
    batch_size: int | None = None,
) -> list[VLLMCompletion]:
    ...
    for prompt_batch in prompt_batches:
        payload = {
            ...
            # sample n=group_size completions for each prompt
            "n": sampling_params["n"],
        ...
        # total number of completions = len(prompts) * n = rollout_batch_size
...
def sync_policy_weights(policy: torch.nn.Module, vllm_base_url: str, weight_sync_group) -> None:
    ...
    # vLLM serves the Qwen3-compatible export, while the trainable policy
    # keeps ajLLM's educational names.  Use the same mapping as the exporter.
    weights = list(iter_qwen3_named_parameters(policy))
    update_info = {
