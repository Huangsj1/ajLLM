...
An educational, compact LLM project with pre-training and supervised fine-tuning
(SFT) workflows built around one clean decoder-only baseline.
...
src/ajllm/datasets/   pre-training and SFT datasets
...
See [architecture](docs/architecture.md) for the model design, [training](docs/training.md) for the pre-training path, [SFT](docs/sft.md) for conversational fine-tuning, and [parallel training](docs/parallel_training.md) for FSDP, TP and EP launches, limits and validation.
